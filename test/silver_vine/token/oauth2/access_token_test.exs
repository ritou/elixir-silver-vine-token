defmodule SilverVine.Token.OAuth2.AccessTokenTest do
  use ExUnit.Case
  doctest SilverVine.Token.OAuth2.AccessToken

  alias SilverVine.Token.OAuth2.AccessToken
  ...
  test "generate, verify" do
    ...
    #generate
    assert {:ok, token} = AccessToken.generate(payload, @key)
    [encoded_header, _, _] = String.split(token, ".")
    assert %{"alg" => "HS256", "kid" => "kid20200914", "typ" => "at+jwt"} == encoded_header |> Base.url_decode64!(padding: false) |> Jason.decode!()

    # verify
    assert {:ok, verified_payload} = AccessToken.verify(token, [@key])
